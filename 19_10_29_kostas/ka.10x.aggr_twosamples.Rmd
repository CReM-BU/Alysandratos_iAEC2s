...
sc.counts <-  Read10X(paste0(cremwd, "/project_workspace/19_10_29_kostas/calculations/cellranger_aggr_twosamples/", prefix, "/outs/filtered_feature_bc_matrix")) # in seu3 no need to gunzip * ; mv features.tsv genes.tsv
...
```{r deg_pairwise, eval=FALSE}
fmas.pair <- function(sc, res, id1, id2) {
    file_out <- paste0(calculations_path, res, ".", id1, ".", id2, ".DEG.clusterMarkers.xlsx")
    seurat.markers <- FindMarkers(sc, test.use = "MAST", only.pos = FALSE, min.pct = 0.10, logfc.threshold = 0.15, verbose = F, group.by = res,  ident.1 = id1, ident.2 = id2)
    seurat.markers.summary <- as.data.frame(seurat.markers) %>%  rownames_to_column('gene') %>% dplyr::filter(p_val_adj < 0.05) %>% arrange(desc(avg_logFC)) %>% filter(row_number() %in% 1:5 | row_number() %in% (n()-4):n())
    openxlsx::write.xlsx(seurat.markers, file_out, zoom = 100, asTable = TRUE, tableStyle = "none", rowNames = T)
    return(list(all = seurat.markers, top = seurat.markers.summary))
}
sc.markers.mas.pair.res1 <- fmas.pair(sc, "orig.ident", id1 = "B2_nov", id2 = "C11_nov")
